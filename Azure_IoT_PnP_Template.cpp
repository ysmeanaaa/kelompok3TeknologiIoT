...
static az_span COMMAND_NAME_TURN_SERVO_45 = AZ_SPAN_FROM_STR("TurnServo45");
static az_span COMMAND_NAME_TURN_SERVO_0 = AZ_SPAN_FROM_STR("OffServo");
static az_span COMMAND_NAME_BUZZER_ON = AZ_SPAN_FROM_STR("BuzzerOn");
static az_span COMMAND_NAME_BUZZER_OFF = AZ_SPAN_FROM_STR("BuzzerOff");
...
static bool servooff = false;
static bool buzzer_logic = false;
...
int azure_pnp_handle_command_request(azure_iot_t* azure_iot, command_request_t command)
{
  ...
  else if (az_span_is_content_equal(command.command_name, COMMAND_NAME_TURN_SERVO_45))
    {
        servo = !servo;
        LogInfo("Servo state: %s", (servo ? "ON" : "OFF"));
        // Call function to move servo to 45 degrees
        move_servo_to_45();
        response_code = COMMAND_RESPONSE_CODE_ACCEPTED;
    }
  else if (az_span_is_content_equal(command.command_name, COMMAND_NAME_TURN_SERVO_0))
    {
        servooff = !servooff;
        LogInfo("Servo state: %s", (servo ? "ON" : "OFF"));
        // Call function to move servo to 45 degrees
  ...
  else if (az_span_is_content_equal(command.command_name, COMMAND_NAME_BUZZER_ON))
    {
        buzzer_logic = !buzzer_logic;
        LogInfo("Buzzer: %s", (buzzer_logic ? "ON" : "OFF"));
        // Call function to move servo to 45 degrees
        buzzer();
        response_code = COMMAND_RESPONSE_CODE_ACCEPTED;
    }
  else if (az_span_is_content_equal(command.command_name, COMMAND_NAME_BUZZER_OFF))
    {
        buzzer_logic = !buzzer_logic;
        LogInfo("Buzzer: %s", (buzzer_logic ? "ON" : "OFF"));
        // Call function to move servo to 45 degrees
...
// Fungsi untuk menggerakkan servo ke 45 derajat
void move_servo_to_45()
{
    myservo.attach(servoPin); // servoPin adalah pin yang terhubung ke servo
    myservo.write(45); // Menggerakkan servo ke 45 derajat
    LogInfo("Servo moved to 45 degrees.");
}

// Fungsi untuk menggerakkan servo ke 45 derajat
void move_servo_to_0()
{
    myservo.attach(servoPin); // servoPin adalah pin yang terhubung ke servo
    myservo.write(0); // Menggerakkan servo ke 45 derajat
    LogInfo("Servo moved to 0 degrees.");
}
    ...
    size_t* payload_buffer_length)
{
  ...
  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR(TELEMETRY_PROP_NAME_DISTANCE));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding distance property name to telemetry payload.");
  rc = az_json_writer_append_double(&jw, distance, DOUBLE_DECIMAL_PLACE_DIGITS);
  EXIT_IF_AZ_FAILED(
      rc, RESULT_ERROR, "Failed adding humidity property value to telemetry payload. ");
